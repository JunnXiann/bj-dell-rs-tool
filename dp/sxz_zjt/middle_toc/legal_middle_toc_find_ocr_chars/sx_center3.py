...
@Author  : zhujiantao
@Version : 1.0
@Desc    : ocr识别对应扣图后，家亮通过字图坐标提取疑似版心列扣图，这里通过版心列文字特征提取正确版心列
1.版心列字数大于3小于14，版心列第一个字符坐标纵坐标大于扣图高度的0.23倍
2.第一个字符必须为千字文
"""
import sys
import json
...
def get_center_page():
    ...
    with open("has_middle_toc_fold_id_list_1.json", "r") as f:
        has_middle_toc_fold_id_list = json.loads(f.read())
        ...
        for page in pages:
            ...
            for column_id in center_columns:
                ...
                if chars:
                    ...
                    if 4 <= len(chars) <= 13 and first_char_y >= first_char_y_threshold:
                        # 第一个字为千字文
                        first_char_txt = chars[0]["ocr_txt"]
                        second_char_txt = chars[1]["ocr_txt"]
                        if first_char_txt in thousand_char_str:
                            # 得到第2个字符下边缘纵坐标
                            second_char_y = chars[1]['y'] + chars[1]['h']
                            # 如果第一个字符纵坐标与第二字符纵坐标大于第一个字符高度，符合版心列格式
                            if True:
                                if chars[1]["ocr_txt"] in number_char_str:
                                    fold_id = page['name']
                                    if first_char_txt not in ["一", "二", "吹"]:
                                        print(f"fold_id={fold_id} 千字文:{middle_txt_str}")
                                        if fold_id not in has_middle_toc_fold_id_list:
                                            # 追加扣id
                                            has_middle_toc_fold_id_list.append(fold_id)

    print(f"合格版心列扣数量={len(has_middle_toc_fold_id_list)}")
    with open("has_middle_toc_fold_id_list_3.json", "w") as f:
        f.write(json.dumps(has_middle_toc_fold_id_list))
