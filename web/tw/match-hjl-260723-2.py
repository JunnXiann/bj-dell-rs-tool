...
def batch_find_match_txt(flag2):
    """ 页数据查找cbeta比对文本"""
   ...
   # cond = {'flag1':2609185}
    ...
    for i, page_name in enumerate(page_names):
        ...
        #page = db.page.find_one({'name': page_name}, {'name': 1, 'sutra_id': 1, 'base_txt': 1 })
...
def batch_apply_match_txt():
    ...
    #cond = {'flag1':153621,'flag2':flag2}
    # cond = { 'flag1': 2609185,'source': {'$in': ['RD-3']}}
    cond = {'flag1': 26091838}
    print(cond)
    ...
    for i, lst in enumerate(split_lst):
        ...
        for i, page in enumerate(pages):
            ...
            if not base_txt or not match_txt:
                continue
            if hlp.prop(page, 'match.status') not in [3,4,5] :  # 已找到
                continue
            try:
                ...
                if r:
                    ...
                    db_work.page.update_one({'name': page['name']}, {'$set': {'chars': page['chars'] }})
                else:
                    logging.info('[e1]%s failed' % page['name'])
